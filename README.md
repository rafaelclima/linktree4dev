# 🌳 LinkTree4Dev  

![React](https://img.shields.io/badge/React-18.2.0-61DAFB?style=for-the-badge&logo=react&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-3.2.4-38B2AC?style=for-the-badge&logo=tailwindcss&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-9.6.1-FFCA28?style=for-the-badge&logo=firebase&logoColor=white)
![Firestore](https://img.shields.io/badge/Firestore-9.6.1-FFA000?style=for-the-badge&logo=firebase&logoColor=white)

🔗 **LinkTree4Dev** é uma aplicação web desenvolvida com **ReactJS**, **Tailwind CSS** e **Firebase**, projetada para simplificar a organização e compartilhamento de links para redes sociais e outras páginas na internet.  

Com uma interface **moderna** e **intuitiva**, qualquer pessoa pode criar sua própria página de links personalizada sem precisar de conhecimento técnico avançado. 🎨✨  

---

## 🚀 **Tecnologias Utilizadas**

- ⚛️ **Front-end:** React + TailwindCSS  
- 🔥 **Back-end:** Firebase + Firestore  

---

## 🌟 **Funcionalidades**

✅ **Autenticação Segura:** Sistema de login utilizando **Firebase Authentication**, garantindo acesso seguro à área administrativa. 🔐  
✅ **Área Administrativa:** Interface intuitiva para gerenciar seus links de maneira prática. 🎛️  
✅ **Cadastro de Links:** Adicione, edite e exclua links para redes sociais, portfólios, GitHub e mais! 🔗  
✅ **Interface Responsiva:** Experiência otimizada para desktop, tablet e smartphone. 📱💻  

---

## 📸 **Screenshots**  

### 🔑 Tela de Login  
![Tela de Login](https://i.ibb.co/j4JJxz4/Captura-de-Tela-2024-04-26-a-s-22-29-58.png)  

### ⚙️ Área do Admin  
![Área do admin](https://i.ibb.co/Rz7M3XN/Captura-de-Tela-2024-04-26-a-s-21-17-42.png)  
![Área do admin](https://i.ibb.co/HzJJSSd/Captura-de-Tela-2024-04-26-a-s-21-18-49.png)  

### 🔗 Link para Redes Sociais  
![Link para as redes sociais](https://i.ibb.co/w0wYNM4/Captura-de-Tela-2024-04-26-a-s-21-19-21.png)  

### 🏡 Página Inicial  
![Página inicial](https://i.ibb.co/YBMHGKD/Captura-de-Tela-2024-04-26-a-s-21-19-41.png)  

---

## 👨‍💻 **Autor**

- 🚀 [@rafaelclima](https://rafaellimadev.site)

---

## 📜 **Licença**

Este projeto está licenciado sob a [MIT License](https://choosealicense.com/licenses/mit/).  
